...
async function resolveImportPath(fromFile, moduleSpecifier) {
  if (!moduleSpecifier.startsWith('.')) {
    throw new Error(`Only relative processor imports are supported in ${workspaceRelative(fromFile)}: ${moduleSpecifier}`)
  }
...
async function parseProcessorRegistry(config) {
  ...
  for (const property of initializer.properties) {
    ...
    if (!moduleSpecifier) {
      throw new Error(`Cannot resolve processor executor import ${executorExpression.text} for ${functionName}.`)
    }
...
export function parseServerProcessorDocComment(rawComment, filePath = 'unknown') {
  const cleaned = cleanJsDoc(rawComment)
  const tagIndex = cleaned.indexOf('@serverProcessorDoc')

  if (tagIndex < 0) {
    throw new Error(`Missing @serverProcessorDoc in ${workspaceRelative(filePath)}.`)
  }

  const jsonText = cleaned.slice(tagIndex + '@serverProcessorDoc'.length).trim()
  const start = jsonText.indexOf('{')
  const end = jsonText.lastIndexOf('}')

  if (start < 0 || end < start) {
    throw new Error(`@serverProcessorDoc in ${workspaceRelative(filePath)} must contain a JSON object.`)
  }

  try {
    return JSON.parse(jsonText.slice(start, end + 1))
  } catch (error) {
    throw new Error(`Invalid @serverProcessorDoc JSON in ${workspaceRelative(filePath)}: ${error.message}`)
  }
}

function leadingServerProcessorDocComment(sourceFile, node, filePath) {
  ...
  const matching = ranges
    ...
    .filter((comment) => comment.includes('@serverProcessorDoc'))
  ...
  if (matching.length > 1) {
    throw new Error(`Multiple @serverProcessorDoc comments found before executor in ${workspaceRelative(filePath)}.`)
  }
...
async function readExecutorDoc(entry) {
  ...
  if (!declaration) {
    throw new Error(`Cannot find processor executor ${entry.executorName} in ${workspaceRelative(entry.sourceFilePath)}.`)
  }

  const comment = leadingServerProcessorDocComment(sourceFile, declaration, entry.sourceFilePath)
  if (!comment) {
    throw new Error(`Missing @serverProcessorDoc for ${entry.functionName} (${entry.executorName}) in ${workspaceRelative(entry.sourceFilePath)}.`)
  }

  return parseServerProcessorDocComment(comment, entry.sourceFilePath)
}

function validateArrayField(doc, fieldName, functionName) {
  if (!Array.isArray(doc[fieldName])) {
    throw new Error(`${functionName} @serverProcessorDoc.${fieldName} must be an array.`)
  }
}
...
function validateRequiredFields(doc, functionName) {
  for (const fieldName of requiredDocFields) {
    if (!Object.prototype.hasOwnProperty.call(doc, fieldName)) {
      throw new Error(`${functionName} @serverProcessorDoc is missing required field: ${fieldName}.`)
    }
  }

  for (const fieldName of ['inputs', 'params', 'outputs', 'errors', 'config', 'runtime', 'examples']) {
    validateArrayField(doc, fieldName, functionName)
  }
}

function validateParamDocs(params, functionName) {
  if (params.length === 0) {
    throw new Error(`${functionName} @serverProcessorDoc.params must describe processor parameter rules.`)
  }

  for (const param of params) {
    if (!isRecord(param) || typeof param.key !== 'string' || !param.key.trim() || typeof param.description !== 'string' || !param.description.trim()) {
      throw new Error(`${functionName} @serverProcessorDoc.params[] must be objects with non-empty key and description.`)
    }
  }
}
...
function validateExamples(examples, functionName) {
  for (const example of examples) {
    const processorConfig = isRecord(example) ? example.processor : undefined
    const documentedName = typeof processorConfig === 'string'
      ? processorConfig
      : isRecord(processorConfig)
        ? processorConfig.processor
        : undefined

    if (typeof documentedName !== 'string' || !documentedName.trim()) {
      throw new Error(`${functionName} @serverProcessorDoc.examples[] must include a processor config.`)
    }

    if (documentedName !== functionName) {
      throw new Error(`${functionName} @serverProcessorDoc.examples[].processor must match the registered functionName.`)
    }
  }
}
...
function validateDocAgainstRegistry(doc, entry) {
  if (!isRecord(doc)) {
    throw new Error(`${entry.functionName} @serverProcessorDoc must be a JSON object.`)
  }
  ...
  validateParamDocs(doc.params, entry.functionName)
...
function normalizeDoc(entry, doc) {
  ...
  return {
    uuid: typeof doc.uuid === 'string' && doc.uuid.trim() ? doc.uuid.trim() : `server-processor-${entry.functionName}`,
    ...
    raw_meta: {
      version: doc.version,
      managedBy: 'import-server-processor-docs.mjs',
...
export async function defaultRegistryConfigs() {
  const coreProcessorsRoot = await firstExistingPath([
    path.resolve(serverRoot, '../mokelay-server-core/src/utils/processors'),
    path.resolve(serverRoot, 'node_modules/mokelay-server-core/src/utils/processors'),
    path.resolve(serverRoot, 'node_modules/mokelay-server-core/dist/utils/processors'),
  ], 'mokelay-server-core processors root')

  return [
    {
      indexPath: await firstExistingPath([
        path.join(coreProcessorsRoot, 'index.ts'),
        path.join(coreProcessorsRoot, 'index.js'),
      ], 'mokelay-server-core processor registry'),
...
export async function collectServerProcessorDocs(options = {}) {
  const registries = options.registries ?? await defaultRegistryConfigs()
  const entries = (await Promise.all(registries.map((registry) => parseProcessorRegistry(registry)))).flat()
  const docs = []
  ...
  for (const entry of entries) {
    if (seenFunctionNames.has(entry.functionName)) {
      throw new Error(`Duplicate registered server processor functionName: ${entry.functionName}.`)
    }
    ...
    if (seenUuids.has(doc.uuid)) {
      throw new Error(`Duplicate server processor doc uuid: ${doc.uuid}.`)
    }
...
async function ensureMysqlTable(connection) {
  await connection.execute(`
    CREATE TABLE IF NOT EXISTS docs_server_processor (
...
async function ensurePostgresTable(sql) {
  ...
  `
  await sql`CREATE INDEX IF NOT EXISTS idx_docs_server_processor_category ON docs_server_processor (category)`
  await sql`CREATE INDEX IF NOT EXISTS idx_docs_server_processor_source_kind ON docs_server_processor (source_kind)`
}
...
async function upsertMysql(connection, docs) {
  const query = `
    INSERT INTO docs_server_processor (
      uuid, function_name, display_name, category, source_kind, source_package, source_file,
      executor_name, description, status, input_schema, param_schema, output_schema, error_schema, config_schema,
  ...
  for (const doc of docs) {
    await sql`
      INSERT INTO docs_server_processor (
        ...
        ${sql.json(doc.input_schema)}, ${sql.json(doc.param_schema)}, ${sql.json(doc.output_schema)}, ${sql.json(doc.error_schema)},
...
async function pruneMysql(connection, docs) {
  ...
  await connection.execute(`DELETE FROM docs_server_processor WHERE uuid NOT IN (${placeholders})`, uuids)
}

async function prunePostgres(sql, docs) {
  const uuids = docs.map((doc) => doc.uuid)
  if (!uuids.length) return
  await sql`DELETE FROM docs_server_processor WHERE uuid NOT IN ${sql(uuids)}`
}

export async function writeServerProcessorDocsToDatabase(docs, prune = false) {
  loadEnv()
...
async function main() {
  ...
  const type = await writeServerProcessorDocsToDatabase(docs, prune)
  console.log(`Imported ${docs.length} server processor docs into ${type} database.`)
}
